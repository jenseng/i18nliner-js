...
for (var key in CallHelpers)
  TranslateCall.prototype[key] = CallHelpers[key];
...
TranslateCall.prototype.validateDefault = function() {
  ...
  if (typeof defaultValue === 'object') {
    ...
    var dKeys;
    if ((dKeys = difference(defaultKeys, this.ALLOWED_PLURALIZATION_KEYS)).length > 0)
      throw "invalid pluralization key(s) on line " + this.line + " (" + dKeys.join(", ") + ")";
    if ((dKeys = difference(this.REQUIRED_PLURALIZATION_KEYS, defaultKeys)).length > 0)
...
TranslateCall.prototype.normalizeArguments = function(args) {
  ...
  var others = this.inferArguments(args);
