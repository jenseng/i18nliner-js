...
TranslateCall.prototype.validateDefault = function () {
  ...
  if (typeof defaultValue === "object") {
    ...
    var dKeys;
    if ((dKeys = Utils.difference(defaultKeys, this.ALLOWED_PLURALIZATION_KEYS)).length > 0) throw new Errors.InvalidPluralizationKey(this.line, dKeys);
    if ((dKeys = Utils.difference(this.REQUIRED_PLURALIZATION_KEYS, defaultKeys)).length > 0) throw new Errors.MissingPluralizationKey(this.line, dKeys);

    for (var k in defaultValue) {
      if (defaultValue.hasOwnProperty(k)) {
        var v = defaultValue[k];
        if (typeof v !== "string") throw new Errors.InvalidPluralizationDefault(this.line);
...
TranslateCall.prototype.normalizeArguments = function (args) {
  if (!args.length) throw new Errors.InvalidSignature(this.line, args);

  var others = this.inferArguments(args.slice(), this);
  var key = this.key = others.shift();
  var options = this.options = others.shift();

  if (others.length) throw new Errors.InvalidSignature(this.line, args);
  if (typeof key !== "string") throw new Errors.InvalidSignature(this.line, args);
  if (options && !this.isObject(options)) throw new Errors.InvalidSignature(this.line, args);
  if (options) {
    this.defaultValue = options.defaultValue;
    delete options.defaultValue;
  }
  if (!this.validDefault(true)) throw new Errors.InvalidSignature(this.line, args);
};
...
TranslateCall.prototype.validateInterpolationValues = function (key, defaultValue) {
  ...
  while ((match = pattern.exec(defaultValue)) !== null) {
    placeholder = match[1];
    if (!(placeholder in options)) throw new Errors.MissingInterpolationValue(this.line, placeholder);
  }
};

TranslateCall.prototype.validateOptions = function () {
  var options = this.options;
  if (typeof this.defaultValue === "object" && (!options || !options.count)) throw new Errors.MissingCountValue(this.line);
  if (options) {
    for (var k in options) {
      if (typeof k !== "string") throw new Errors.InvalidOptionKey(this.line);
    }
  }
};
